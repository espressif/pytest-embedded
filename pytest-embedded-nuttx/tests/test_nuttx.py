...
def test_nuttx_app(testdir):
    ...
    result = testdir.runpytest(
        ...
        'esp32',
        '--app-path',
        os.path.join(testdir.tmpdir, 'hello_world_nuttx'),
    )
...
def test_nuttx_app_qemu(testdir):
    ...
    result = testdir.runpytest(
        ...
        'esp32',
        '--app-path',
        os.path.join(testdir.tmpdir, 'hello_world_nuttx_qemu'),
        '--qemu-image-path',
        os.path.join(testdir.tmpdir, 'hello_world_nuttx_qemu', 'nuttx.merged.bin'),
        '--qemu-extra-args',
        qemu_extra,
        '--qemu-prog-path',
        'qemu-system-xtensa',
